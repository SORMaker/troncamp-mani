...
class Robot:
    ...
    def init_joints(self):
        ...
        # gripper_name feeds contact detection (get_gripper_actor_contact_position): it must list the
        # physical finger links that actually carry collision geometry. For a linkage gripper the
        # coupling list also contains massless read-only ref joints (child = *_ref_Link) and internal
        # linkage members (crank/bar/nut) that have no <collision>; letting a ref link into gripper_name
        # would silently defeat contact detection. So only keep coupling children that have collision
        # shapes (the physical jaw links). For simple (base+mimic) grippers whose children are the
        # fingers this is a no-op.
        def _collect_contact_links(gripper):
            for g in gripper:
                child = g[0].child_link
                shapes = child.get_collision_shapes() if hasattr(child, "get_collision_shapes") else []
                if len(shapes) == 0:
                    continue
                self.gripper_name.append(child.get_name())

        _collect_contact_links(self.left_gripper)
        _collect_contact_links(self.right_gripper)
