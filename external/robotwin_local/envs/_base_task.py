...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        # Settle the arms physically to the homestate. move_to_homestate() only sets the PD drive
        # targets; the policy eval starts the rollout immediately (no expert planned-moves to drive
        # the arms there), so without this the arms stay at the loaded ~0 pose while obs/commands
        # sit at the homestate (~0.85 rad) -> the model's first action leaps the arm across the full
        # gap and flings the objects. Collection settles via the expert's first plan; eval must here.
        # GATED to eval only (TRON2_EVAL_SETTLE=1, set by run_act_eval): eval applies the Tron2
        # self-collision patch first, so raw-stepping is safe. Collection does NOT apply the patch,
        # so raw-stepping here lets the arm self-collide off the homestate and corrupts the expert's
        # first grasp plan (target_pose cannot be None for move action) -> 0% collection.
        import os as _os_settle
        if _os_settle.environ.get("TRON2_EVAL_SETTLE") == "1":
            for _ in range(250):
                self.scene.step()
    ...
    def load_robot(self, **kwags):
        ...
        for link in self.robot.left_entity.get_links():
            ...
            if "grasper" in link.get_name() and not link.get_collision_shapes():
                continue  # L4 hybrid: keep the visual-only linkage (crank/bar/nut) near-zero mass
            link.set_mass(1)
    ...
    def _take_picture(self):  # save data
        ...
        if self.FRAME_IDX == 0:
            # One line per episode. The stock per-frame print (end="\r") floods logs with
            # thousands of carriage-return lines under single-pass save (save_data on during
            # the search phase), where stdout is a file rather than a terminal.
            print(f"saving: episode = {self.ep_num}")
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee']='qpos'):  # action_type: qpos or ee
        ...
        # FIX: plan the TOPP move from the REAL physical joint positions, not the drive targets.
        # get_*_arm_jointState() returns set_drive_target() (the commanded setpoint). After
        # move_to_homestate() the setpoint is the homestate (~0.85 rad) but the physical arm is
        # still near 0 until it settles, so planning from the setpoint makes the first PD command
        # leap the arm across the full ~0.85 rad gap in one TOPP -> violent motion that flings the
        # objects. The scripted expert plans from get_qpos() (real), so it stays smooth; the policy
        # take_action path must do the same or the model's correct small actions execute violently.
