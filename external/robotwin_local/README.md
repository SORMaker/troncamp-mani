> ⚠️ **本文件为上游 RoboTwin 平台通用文档,不代表 TronCamp 参赛口径。** TronCamp 选手请以**仓库根
> [`README.md`](../../README.md) ＋ 参赛文档官网**为准:四任务数据**全部自采**(不下发预采数据集)、
> 安装用 `bash setup/install.sh`、任务与 config 见根 README。以下为**上游 RoboTwin 原版 README**,
> 仅作平台参考(其中的示例命令 / 预采数据集 / 上游安装步骤等均属上游语境,勿照搬到 TronCamp 流程)。
