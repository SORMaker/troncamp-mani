#!/usr/bin/env bash
# T2 grab_roller: named small ACT baseline profile.
set -euo pipefail
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export ACT_TRACK=T2
export ACT_PROFILE="${ACT_PROFILE:-T2-h512-c50-ff3200}"
...
seed=${1:-0}
gpu_ids=${2:-0}
exec bash "$script_dir/train.sh" grab_roller grab_roller_200ep 200 "$seed" "$gpu_ids" "${@:3}"
